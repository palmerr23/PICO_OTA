...
#else // SDFS - not yet supported.
  //SPI.begin() and FILESYS.setCSPin(CSPIN) are3 required. 
	//#include "FS.h"
  #include <SDFS.h>
	#define FILESYS SDFS
  char fsName[] = "SDFS";
...
void handleMain() {
  ...
  File dir = FILESYS.open(path.c_str(),"r");
  if(!dir)
 ...
 deleteOTAfiles();
  // Create HTML page
  output = "<html><head>";
  output += penguinIco; // favicon.ico
  output += "</head><body onload='scrollToBottom(\"log\")'>\n";
  output += "<span style='text-align: center;'><h2>PICO W OTA and File Management</H2></span>";
  ...
  // FS format 
   ...
   output += "<button>Re-format FS</button>";
   output += "</form></span>";  
   output += "</td></tr>\n";
   ...
   output += "Path: <input type='text' name='path' value='/' size='36'>";
...
{
  String path;
  Serial.println("Deleting 'firmware.bin' and 'otacommand.bin'");
